...
describe("GET /api/guilds", () => {
  ...
  it("returns 500 when mutual guild lookup times out", async () => {
    mockGetToken.mockResolvedValue({
      sub: "123",
      accessToken: "valid-discord-token",
      id: "discord-user-123",
    });
    mockGetMutualGuilds.mockRejectedValue(new DOMException("Timed out", "TimeoutError"));
  ...
  it("passes an empty userId when token identity fields are absent", async () => {
    globalThis.fetch = vi.fn();
