...
- Volvox.Bot grants ticket access to all selected roles. In channel-mode tickets, each role gets a permission overwrite. In thread-mode tickets, Volvox.Bot automatically adds members of every selected role, adding each member only once even if they hold multiple roles.
- Volvox.Bot automatically skips stale role IDs (roles that no longer exist on the server).
- Discord caps channel permission overwrites at 100. Volvox.Bot reserves three for `@everyone`, the ticket opener, and itself, so it applies up to **97 support roles** per channel-mode ticket and ignores any extras.
