...
- Open **Config → Tickets** and use the **Support Staff Roles** picker to add every role that should see new tickets.
  - Volvox.Bot grants ticket access to all selected roles. In channel-mode tickets, each role gets a permission overwrite. In thread-mode tickets, Volvox.Bot adds members of every selected role only once, even if they hold multiple roles.
