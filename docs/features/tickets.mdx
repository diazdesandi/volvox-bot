...
Volvox.Bot supports two ticket modes. Pick the one that fits how your team works.
...
Channel-mode tickets require the bot's **Manage Channels** permission. Thread-mode tickets do not.
...
- Open **Config → Tickets** and use the **Support Staff Roles** picker to add every role that should see new tickets.
  ...
  - Discord caps channel permission overwrites at 100. The bot reserves three for `@everyone`, the ticket opener, and itself. That leaves up to **97 support roles** per channel-mode ticket; extras are ignored.
