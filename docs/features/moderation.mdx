...
These are advanced tuning knobs; defaults work for most servers. Use the server config fields below to understand the current AI triage latency defaults. The dashboard UI does not currently expose these fields. Config loading normalizes each value and clamps it to the listed range. The runtime cooldown gate currently enforces `responseCooldownMs`.
